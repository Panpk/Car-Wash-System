...
public class TableViewCreation extends TableView<VehicleInfo>{
    ...
    public ObservableList<VehicleInfo> addVehicleInfo(String licensePlate, String cost, String currentTime) {
        ObservableList<VehicleInfo> data = tableView.getItems();
        if (data == null) {
            data = FXCollections.observableArrayList();
            tableView.setItems(data);
        }
        VehicleInfo newItem = new VehicleInfo(licensePlate, cost, currentTime);
        data.add(newItem);
        return data;
    }   
